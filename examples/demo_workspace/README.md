...
lifecycle review examples. Dataset/descriptor v7 also supplies one exact
...
Demo v7 preserves the existing Paper Account application-service journey and
...
Demo v7 additionally preserves the existing immutable XNYS calendar, sessions,
...
The installer preserves the original Strategy-to-Risk journey, then builds five
...
The fifth handoff uses the same services to create one fresh nonterminal M34
Order and binds exactly one M35 Paper Runtime through
`PaperRuntimeLifecycleService`. It starts stopped/ready/unowned with no Work,
checkpoint, Attempt, Fill, settlement, or runtime-driven replay progression.
Its future events deterministically produce one different-instrument no-Fill,
one matching partial Fill with one settlement, and one terminal out-of-session
boundary rejection without another Fill or cursor advance. Runtime execution is
performed only by the external `run-paper-runtime` process seam.
...
rows, advances replay, or mutates the Paper Account. Demo v7 contains no broker,
live, reservation, scheduler, HTTP runner, or real-money authority.
