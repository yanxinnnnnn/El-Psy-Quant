...
is unchanged. Through the existing S202 application/domain/repository path, the
same exact M31 account and M32 replay authority produce one Signal, one Intent,
one allow decision, and one reject decision. Descriptor metadata remains
non-authoritative. This verification does not create orders, execution, or
account state.
...
explicitly reset the disposable earlier-version Demo volume when required
  ...
  -> descriptor v5 is visible and clearly labeled
  ...
  -> Market Time shows the exact paused replay and five ordered events
  -> Strategy to Risk shows the exact Signal, Intent, allow, and reject evidence
  -> restart preserves every M33 identity/digest and the replay cursor at position 4
