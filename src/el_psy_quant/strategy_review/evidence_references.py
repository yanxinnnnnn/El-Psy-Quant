...
STRATEGY_REVIEW_EVIDENCE_REFERENCE_SCHEMA_VERSION = 1
...
def _normalize_reference_type(reference_type: str) -> str:
    ...
    if normalized not in SUPPORTED_STRATEGY_REVIEW_EVIDENCE_REFERENCE_TYPES:
        supported = ", ".join(SUPPORTED_STRATEGY_REVIEW_EVIDENCE_REFERENCE_TYPES)
        raise ValueError(
...
class StrategyReviewEvidenceReference:
    """Immutable pointer to completed strategy review evidence."""
...
def create_strategy_review_evidence_reference(
    *,
    reference_type: str,
    reference_id: str,
    label: str | None = None,
    ...
    """Create and validate one strategy review evidence reference."""
    return StrategyReviewEvidenceReference(
