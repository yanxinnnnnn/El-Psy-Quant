...
export function PaperRuntimeDetailView({ runtimeId }: { runtimeId: string }) {
  ...
  async function loadMoreAudit() {
    ...
    try { const result = await fetchPaperRuntimeAudit(runtimeId, { limit: 25, cursor: audit.next_cursor }); if (sequence === auditSequence.current) { setAudit({ ...result.data, items: [...audit.items, ...result.data.items] }); clearFailure("audit"); } } catch (caught) { if (sequence === auditSequence.current) setSectionFailure("audit", caught); }
  }
  async function loadMoreWork() {
    if (!work?.next_cursor) return;
    const sequence = ++workSequence.current;
    try { const result = await fetchPaperRuntimeWork(runtimeId, { limit: 25, cursor: work.next_cursor }); if (sequence === workSequence.current) { setWork({ ...result.data, items: [...work.items, ...result.data.items] }); clearFailure("work"); } } catch (caught) { if (sequence === workSequence.current) setSectionFailure("work", caught); }
  }
  async function loadMoreCheckpoints() {
    if (!checkpoints?.next_cursor) return;
    const sequence = ++checkpointSequence.current;
    try { const result = await fetchPaperRuntimeCheckpoints(runtimeId, { limit: 25, cursor: checkpoints.next_cursor }); if (sequence === checkpointSequence.current) { setCheckpoints({ ...result.data, items: [...checkpoints.items, ...result.data.items] }); clearFailure("checkpoints"); } } catch (caught) { if (sequence === checkpointSequence.current) setSectionFailure("checkpoints", caught); }
  }
  ...
  return <div className="business-workspace">
    ...
    <section className="content-panel" aria-labelledby="runtime-health"><div className="section-heading"><div><p className="eyebrow">{t("detail.healthEyebrow")}</p><h2 id="runtime-health">{t("detail.healthTitle")}</h2></div><p>{t("detail.healthDescription")}</p></div><SectionFailure value={failures.health} operation="paper_runtime.health" onRetry={() => void refresh(false)} />{health ? <dl className="definition-grid definition-grid--wide"><div><dt>{t("fields.claimed")}</dt><dd><Raw value={health.claimed} /></dd></div><div><dt>{t("fields.lease_status")}</dt><dd><Raw value={health.lease_status} /></dd></div><div><dt>{t("fields.owner_id")}</dt><dd><Raw value={runtime?.owner_id ?? null} /></dd></div><div><dt>{t("fields.claimed_at")}</dt><dd>{health.claimed_at ? <LocalizedTimestamp value={health.claimed_at} /> : <Raw value={null} />}</dd></div><div><dt>{t("fields.heartbeat_at")}</dt><dd>{health.heartbeat_at ? <LocalizedTimestamp value={health.heartbeat_at} /> : <Raw value={null} />}</dd></div><div><dt>{t("fields.lease_expires_at")}</dt><dd>{health.lease_expires_at ? <LocalizedTimestamp value={health.lease_expires_at} /> : <Raw value={null} />}</dd></div><div><dt>{t("fields.terminal")}</dt><dd><Raw value={health.terminal} /></dd></div><div><dt>{t("fields.blocked")}</dt><dd><Raw value={health.blocked} /></dd></div><div><dt>{t("fields.checked_at")}</dt><dd><LocalizedTimestamp value={health.checked_at} /></dd></div></dl> : null}</section>
