...
function ScenarioProse({
  ...
  title: string;
  scenario:
    | Detail["analysis"]["baseline_scenario"]
    | Detail["analysis"]["proposed_scenario"];
}) {
  const common = useTranslations("portfolioReviews.common");
  ...
  return (
    <article className="evidence-card">
      ...
      <dl className="definition-grid">
        <div><dt>{common("schemaVersion")}</dt><dd><RawValue value={scenario.schema_version} /></dd></div>
        <div><dt>{fields("scenarioId")}</dt><dd><RawValue value={scenario.scenario_id} /></dd></div>
        <div><dt>{fields("sourceId")}</dt><dd><RawValue value={scenario.source_id} /></dd></div>
        <div><dt>{common("sourceDigest")}</dt><dd><RawValue value={scenario.source_digest} /></dd></div>
        <div><dt>{common("scenarioDigest")}</dt><dd><RawValue value={scenario.scenario_digest} /></dd></div>
        {"proposed_component_id" in scenario ? (
          <div><dt>{fields("proposedComponent")}</dt><dd><RawValue value={scenario.proposed_component_id} /></dd></div>
        ) : null}
        <div><dt>{fields("rationale")}</dt><dd>{scenario.rationale}</dd></div>
        <div><dt>{common("assumptions")}</dt><dd><RawStringList values={scenario.assumptions} /></dd></div>
        <div><dt>{common("warnings")}</dt><dd><RawStringList values={scenario.warnings} /></dd></div>
      </dl>
    </article>
  );
}
...
function ConcentrationTable({
  ...
  const rows = [
    ["schemaVersion", analysis.baseline_concentration.schema_version, analysis.proposed_concentration.schema_version],
    ["scenarioId", analysis.baseline_concentration.scenario_id, analysis.proposed_concentration.scenario_id],
    ["scenarioDigest", analysis.baseline_concentration.scenario_digest, analysis.proposed_concentration.scenario_digest],
      ...
      <table>
        ...
        <tbody>
          {rows.map(([key, baseline, proposed]) => (
            <tr key={key}>
              <th scope="row">{key === "schemaVersion" ? common("schemaVersion") : key === "scenarioId" ? fields("scenarioId") : key === "scenarioDigest" ? common("scenarioDigest") : metrics(key)}</th>
...
function CoverageCard({ title, value }: { title: string; value: Coverage }) {
  ...
  return (
    <article className="evidence-card">
      <h3>{title}</h3>
      <dl className="definition-grid">
        <div><dt>{common("schemaVersion")}</dt><dd><RawValue value={value.schema_version} /></dd></div>
        <div><dt>{fields("scenarioId")}</dt><dd><RawValue value={value.scenario_id} /></dd></div>
        <div><dt>{common("scenarioDigest")}</dt><dd><RawValue value={value.scenario_digest} /></dd></div>
        <div><dt>{metrics("sourceComponentCount")}</dt><dd><RawValue value={value.source_component_count} /></dd></div>
        <div><dt>{metrics("componentsWithEvidenceCount")}</dt><dd><RawValue value={value.components_with_symbol_evidence_count} /></dd></div>
        <div><dt>{metrics("componentsMissingEvidenceCount")}</dt><dd><RawValue value={value.components_missing_symbol_evidence_count} /></dd></div>
        <div><dt>{metrics("activeComponentCount")}</dt><dd><RawValue value={value.active_component_count} /></dd></div>
        <div><dt>{metrics("activeWithEvidenceCount")}</dt><dd><RawValue value={value.active_components_with_symbol_evidence_count} /></dd></div>
        <div><dt>{metrics("activeMissingEvidenceCount")}</dt><dd><RawValue value={value.active_components_missing_symbol_evidence_count} /></dd></div>
        ...
        <div><dt>{metrics("activeComponentIds")}</dt><dd><RawStringList values={value.active_component_ids} /></dd></div>
...
function behaviorValue(behavior: Behavior, key: (typeof behaviorMetricKeys)[number]) {
  const mapping = {
    schemaVersion: behavior.schema_version,
    scenarioId: behavior.scenario_id,
    scenarioDigest: behavior.scenario_digest,
    observationCount: behavior.observation_count,
    evaluationStart: behavior.evaluation_start_timestamp,
    evaluationEnd: behavior.evaluation_end_timestamp,
    ...
    (typeof behaviorMetricKeys)[number],
    string | number | null
  >;
  return mapping[key];
}
...
function ContributionTable({
  ...
  title: string;
  values: Behavior["component_contributions"];
}) {
  const fields = useTranslations("portfolioReviews.fields");
  const metrics = useTranslations("portfolioReviews.metrics");
  const common = useTranslations("portfolioReviews.common");
  return (
    <div className="table-scroll">
      <table>
        ...
        <thead>
          <tr>
            <th scope="col">{fields("componentId")}</th>
            <th scope="col">{common("schemaVersion")}</th>
            <th scope="col">{fields("strategyId")}</th>
            <th scope="col">{metrics("weight")}</th>
            <th scope="col">{metrics("totalContribution")}</th>
            <th scope="col">{metrics("meanContribution")}</th>
            <th scope="col">{metrics("positivePeriods")}</th>
            <th scope="col">{metrics("negativePeriods")}</th>
            <th scope="col">{metrics("zeroPeriods")}</th>
          </tr>
        </thead>
        <tbody>
          {values.map((item, index) => (
            <tr key={`${item.component_id}-${index}`}>
              ...
              <td><RawValue value={item.schema_version} /></td>
              <td><RawValue value={item.strategy_id} /></td>
              <td><RawValue value={item.weight} /></td>
              <td><RawValue value={item.total_contribution} /></td>
              <td><RawValue value={item.mean_contribution} /></td>
              <td><RawValue value={item.positive_periods} /></td>
              <td><RawValue value={item.negative_periods} /></td>
              <td><RawValue value={item.zero_periods} /></td>
            </tr>
          ))}
        </tbody>
      </table>
    ...
    <>
      <EvidenceSection id="source" title={t("source")} description={t("componentOrder")}>
        <dl className="definition-grid definition-grid--wide">
          <div><dt>{common("schemaVersion")}</dt><dd><RawValue value={source.schema_version} /></dd></div>
        ...
        <ol className="portfolio-component-inspection">
          {source.components.map((component, index) => (
            <li className="evidence-card" key={`${component.component_id}-${index}`}>
              ...
              <dl className="definition-grid">
                <div><dt>{common("schemaVersion")}</dt><dd><RawValue value={component.schema_version} /></dd></div>
                <div><dt>{fields("componentId")}</dt><dd><RawValue value={component.component_id} /></dd></div>
      ...
      <EvidenceSection id="observations" title={t("observations")} description={t("fullObservations")}>
        <div className="table-scroll">
          <table>
            ...
            <thead><tr><th scope="col">#</th><th scope="col">{common("schemaVersion")}</th><th scope="col">{fields("timestamp")}</th>{source.components.map((component) => <th scope="col" key={component.component_id}>{component.component_id}</th>)}</tr></thead>
            <tbody>
              {source.return_observations.map((observation, index) => (
                <tr key={`${index}-${observation.timestamp}`}>
                  <th scope="row">{index + 1}</th>
                  <td><RawValue value={observation.schema_version} /></td>
      ...
      <EvidenceSection id="scenarios" title={t("scenarios")} description={t("scenarioWeights")}>
        <div className="evidence-pair">
          <ScenarioProse title={t("baseline")} scenario={baseline} />
          <ScenarioProse title={t("proposed")} scenario={proposed} />
        </div>
        <div className="table-scroll">
          <table>
      ...
      <EvidenceSection id="concentration" title={t("concentration")} description={t("concentrationEvidence")}>
        <dl className="definition-grid definition-grid--wide evidence-card">
          <div><dt>{common("schemaVersion")}</dt><dd><RawValue value={concentration.schema_version} /></dd></div>
          <div><dt>{fields("sourceId")}</dt><dd><RawValue value={concentration.source_id} /></dd></div>
          <div><dt>{common("sourceDigest")}</dt><dd><RawValue value={concentration.source_digest} /></dd></div>
          <div><dt>{t("componentOrder")}</dt><dd><RawStringList values={concentration.component_ids} /></dd></div>
        </dl>
      ...
      <EvidenceSection id="exposure" title={t("exposure")} description={t("exposureEvidence")}>
        <div className="table-scroll">
          <table>
            ...
            <thead><tr><th scope="col">{common("schemaVersion")}</th><th scope="col">{fields("componentId")}</th><th scope="col">{fields("strategyId")}</th><th scope="col">{fields("baselineWeight", { componentId: "" })}</th><th scope="col">{fields("proposedWeight", { componentId: "" })}</th><th scope="col">{metrics("weightDelta")}</th><th scope="col">{metrics("changeType")}</th><th scope="col">{metrics("baselineActive")}</th><th scope="col">{metrics("proposedActive")}</th><th scope="col">{metrics("symbolEvidenceStatus")}</th><th scope="col">{t("symbols")}</th></tr></thead>
            <tbody>
              {concentration.component_exposures.map((exposure, index) => (
                <tr key={`${exposure.component_id}-${index}`}>
                  <td><RawValue value={exposure.schema_version} /></td>
                  ...
                  <td><RawValue value={exposure.baseline_active} /></td>
                  <td><RawValue value={exposure.proposed_active} /></td>
      ...
      <EvidenceSection id="overlap" title={t("overlap")} description={t("overlapEvidence")}>
        <dl className="definition-grid definition-grid--wide evidence-card">
          <div><dt>{common("schemaVersion")}</dt><dd><RawValue value={interaction.schema_version} /></dd></div>
          <div><dt>{fields("sourceId")}</dt><dd><RawValue value={interaction.source_id} /></dd></div>
          <div><dt>{common("sourceDigest")}</dt><dd><RawValue value={interaction.source_digest} /></dd></div>
          <div><dt>{t("componentOrder")}</dt><dd><RawStringList values={interaction.component_ids} /></dd></div>
          <div><dt>{fields("proposedComponent")}</dt><dd><RawValue value={interaction.proposed_component_id} /></dd></div>
        </dl>
        <ol className="evidence-list">
          {interaction.symbol_overlaps.map((overlap, index) => (
            <li className="evidence-card" key={`${index}-${overlap.left_component_id}-${overlap.right_component_id}`}>
              ...
              <dl className="definition-grid">
                <div><dt>{common("schemaVersion")}</dt><dd><RawValue value={overlap.schema_version} /></dd></div>
                <div><dt>{metrics("leftComponent")}</dt><dd><RawValue value={overlap.left_component_id} /></dd></div>
                <div><dt>{metrics("rightComponent")}</dt><dd><RawValue value={overlap.right_component_id} /></dd></div>
                <div><dt>{t("status")}</dt><dd><RawValue value={overlap.status} /></dd></div>
                <div><dt>{t("reason")}</dt><dd><RawValue value={overlap.unavailable_reason} /></dd></div>
                <div><dt>{t("affectedComponents")}</dt><dd><RawStringList values={overlap.missing_symbol_component_ids} /></dd></div>
      ...
      <EvidenceSection id="correlation" title={t("correlation")} description={t("pairwiseEvidence")}>
        <ol className="evidence-list">
          {interaction.pairwise_correlations.map((pair, index) => (
            <li className="evidence-card" key={`${index}-${pair.left_component_id}-${pair.right_component_id}`}>
              ...
              <dl className="definition-grid">
                <div><dt>{common("schemaVersion")}</dt><dd><RawValue value={pair.schema_version} /></dd></div>
                <div><dt>{metrics("leftComponent")}</dt><dd><RawValue value={pair.left_component_id} /></dd></div>
                <div><dt>{metrics("rightComponent")}</dt><dd><RawValue value={pair.right_component_id} /></dd></div>
                <div><dt>{t("status")}</dt><dd><RawValue value={pair.status} /></dd></div>
                <div><dt>{t("reason")}</dt><dd><RawValue value={pair.unavailable_reason} /></dd></div>
                <div><dt>{t("zeroVarianceSeries")}</dt><dd><RawStringList values={pair.zero_variance_series} /></dd></div>
        ...
        <article className="evidence-card candidate-correlation">
          ...
          <dl className="definition-grid">
            <div><dt>{common("schemaVersion")}</dt><dd><RawValue value={interaction.candidate_baseline_correlation.schema_version} /></dd></div>
            ...
            <div><dt>{metrics("baselineScenarioId")}</dt><dd><RawValue value={interaction.candidate_baseline_correlation.baseline_scenario_id} /></dd></div>
            <div><dt>{metrics("baselineScenarioDigest")}</dt><dd><RawValue value={interaction.candidate_baseline_correlation.baseline_scenario_digest} /></dd></div>
            <div><dt>{t("status")}</dt><dd><RawValue value={interaction.candidate_baseline_correlation.status} /></dd></div>
            <div><dt>{t("reason")}</dt><dd><RawValue value={interaction.candidate_baseline_correlation.unavailable_reason} /></dd></div>
            <div><dt>{t("zeroVarianceSeries")}</dt><dd><RawStringList values={interaction.candidate_baseline_correlation.zero_variance_series} /></dd></div>
            ...
            <div><dt>{metrics("evaluationStart")}</dt><dd><RawValue value={interaction.candidate_baseline_correlation.evaluation_start_timestamp} /></dd></div>
            <div><dt>{metrics("evaluationEnd")}</dt><dd><RawValue value={interaction.candidate_baseline_correlation.evaluation_end_timestamp} /></dd></div>
          </dl>
        ...
        <div className="evidence-pair">
          {([
            ...
            <article className="evidence-card" key={title}>
              ...
              <dl className="definition-grid">
                <div><dt>{common("schemaVersion")}</dt><dd><RawValue value={drawdown.schema_version} /></dd></div>
      ...
      <EvidenceSection id="contribution" title={t("contribution")} description={t("contributionEvidence")}>
        <ContributionTable
          title={`${t("baseline")} · ${t("contributionEvidence")}`}
          values={interaction.baseline_behavior.component_contributions}
        />
        <ContributionTable
          title={`${t("proposed")} · ${t("contributionEvidence")}`}
          values={interaction.proposed_behavior.component_contributions}
        />
      </EvidenceSection>
      ...
      <EvidenceSection id="impact" title={t("impact")} description={t("impactEvidence")}>
        <dl className="definition-grid definition-grid--wide evidence-card">
          {([
            ["schemaVersion", interaction.proposed_impact.schema_version],
        ...
        <div className="table-scroll">
          <table>
            ...
            <thead><tr><th scope="col">{fields("componentId")}</th><th scope="col">{common("schemaVersion")}</th><th scope="col">{fields("strategyId")}</th><th scope="col">{metrics("baselineWeight")}</th><th scope="col">{metrics("proposedWeight")}</th><th scope="col">{metrics("totalContributionDelta")}</th><th scope="col">{metrics("meanContributionDelta")}</th><th scope="col">{metrics("positivePeriodsDelta")}</th><th scope="col">{metrics("negativePeriodsDelta")}</th><th scope="col">{metrics("zeroPeriodsDelta")}</th></tr></thead>
            <tbody>
              {interaction.component_contribution_impacts.map((item, index) => (
                <tr key={`${item.component_id}-${index}`}>
                  ...
                  <td><RawValue value={item.schema_version} /></td>
                  <td><RawValue value={item.strategy_id} /></td>
                  <td><RawValue value={item.baseline_weight} /></td>
                  <td><RawValue value={item.proposed_weight} /></td>
