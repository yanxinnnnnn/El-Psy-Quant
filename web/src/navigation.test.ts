...
describe("workspace navigation", () => {
  it("enables only Overview and Strategies and Research", () => {
    ...
    expect(workspaceDestinations.slice(2).every((item) => item.href === undefined)).toBe(
      true,
    );
  });

  it("keeps S154 through S158 explicit and unavailable", () => {
    expect(workspaceDestinations.slice(2).map(({ label, sprint, available }) => ({
      label,
      sprint,
      available,
    }))).toEqual([
